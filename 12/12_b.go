...
func get_start_points(map_grid [][]int) []Pair {
	start_points := []Pair{}

	for i := range map_grid {
		for j := range map_grid[i] {
			if map_grid[i][j] == 0 {
				start_points = append(start_points, Pair{j, i})
			}
		}
	}
...
func main() {
	...
	start_points := get_start_points(map_grid)
	min_path_cost := 0

	fmt.Println(start_points)

	for i, sp := range start_points {
		path_cost := dijsktra(sp, exit, map_grid)
		if path_cost == 0 {
			continue
		}

		if i == 0 || path_cost < min_path_cost {
			min_path_cost = path_cost
		}
	}

	fmt.Println(min_path_cost)
}
